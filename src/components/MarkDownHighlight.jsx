    ...
    <div className="space-y-4 w-full">
      {parts.map((part, idx) => {
        if (part.startsWith('```')) {
          ...
          return (
            <CodeEditor key={idx} code={raw} language={language} />
          );
        } else {
          return (
            <p key={idx} 
               className="text-base w-full leading-relaxed overflow-x-hidden [&_a]:text-blue-400 [&_a]:hover:text-blue-300" 
               dangerouslySetInnerHTML={{ __html: marked(part).replace(/\n$/, '') }}></p>
          );
        }
      })}
    </div>
...
const CodeEditor = ({ code, language }) => {
  ...
  return (
    <div className="border rounded-lg overflow-hidden bg-black/30">
      {/* Display language as header */}
